...
These events will accept a function much the same as an **Event Listener**. You can use an anonymous function, or you can write the function and then pass it to the event. In React, a common naming convention is to add "handle" to the function names that are "handling" the events. The event object can be passed as a parameter.
...
React re-renders when a state changes. We can use console logs to track when it happens.
