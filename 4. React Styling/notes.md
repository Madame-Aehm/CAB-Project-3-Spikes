...
If you log this variable to the console, you'll see that it's an object. The keys are the classnames you define in the Module, the values are unique strings generated by the Module to refer to your class. When applying the style, you'll put `variable.classname` into the `className` property on your element. This is what makes CSS Modules so useful - you can use the same classname multiple times (in _different_ Modules), but even imported into the same Component the classname actually applied to your element will always be unique, avoiding any classname style conflicts. 

If you want to use CSS classes to do styling, make sure to use JSX `className`, rather than HTML `class` when applying them to your elements. CSS classes are always strings, so if you want to add multiple classes from your CSS Module, you will have to use template literals or string concatenation.
