...
Styling with `.css` files in React is a bit different than with a MPA. If you linked the `.css` stylesheet to the single `.html` file, then it will apply to every Component, since all Components render on the same document, so `.css` files are **imported** in order to **scope** the styles to that Component. However because of the nested nature of Components, CSS styles applied to the parent will also apply to the children, so avoiding conflict with selectors can be tricky. If you still want to use CSS classes to do styling, make sure to use JSX `className`, rather than HTML `class` when applying them.

React also allows [**inline styling**](https://www.w3schools.com/react/react_css.asp) with the `style` property. This property takes a JavaScript object - key/value pairs will represent the CSS property to be styled and value, however JSX will need you to use **camelCase**. Writing the style directly into the property is helpful as VSCode will recognise and make suggestions for you. Once you have it styled how you like, if you want to clean your JSX and seperate your style object, you can create a variable to hold that value and then pass that variable to the `style` property. If you choose to style this way, you can create a seperate `styles.js` file and export all these variables to be used where you need them across your app. 

If you would like to use a CSS library, they will usually have an npm package that you can install. They will then have finished React Components that you can import and use.